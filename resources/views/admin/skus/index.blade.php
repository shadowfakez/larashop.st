...
@section('header')
    Skus of {{ $product->name }}
@endsection

@section('admin-content')
    <div class="py-5 mb-6">
        <div class='overflow-x-auto w-full'>
            <a href="{{ route('admin.skus.create', $product) }}" type="button"
               class="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-6">Add New Sku</a>
            <table
                class='mx-auto w-full whitespace-nowrap rounded-lg bg-white divide-y divide-gray-300 overflow-hidden'>
                <thead class="bg-gray-900">
                <tr class="text-white text-left">
                    <th class="font-semibold text-sm uppercase px-6 py-4"> #</th>
                    <th class="font-semibold text-sm uppercase px-6 py-4"> Product</th>
                    <th class="font-semibold text-sm uppercase px-6 py-4"> Sku properties</th>
                ...
                @foreach($skus as $sku)
                    <tr class="border-b">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            <a href=""
                               class="hover:text-rose-600">{{ $sku->id }}</a>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            <a href=""
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                            <a href=""
                               class="hover:text-rose-600">{{ $sku->propertyOptions->map->name->implode(', ') }}</a>
                        </td>
                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                            <a type="button" href="{{ route('admin.skus.show', [$product, $sku->id]) }}"
                        ...
                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                            <a type="button" href="{{ route('admin.skus.edit', [$product, $sku->id]) }}"
                        ...
                        <td class="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                            <form action="{{ route('admin.skus.destroy', [$product, $sku]) }}" method="POST"
                                ...
                                <button type="submit"
                                        onclick="return confirm('You want to delete sku! Please, confirm your action!')"
